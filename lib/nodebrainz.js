...
class BaseNodeBrainz {
  ...
  luceneSearch(type, data, callback) {
    ...
    // See if dismax in the search fields
    if (data.hasOwnProperty('dismax')) {
      // Set dismax
      dismax = data.dismax;
    }

    // Set the path
    this.path = this.basePath + type + '/?query=' + query + '&limit=' + limit + '&offset=' + offset + + '&dismax=' + dismax + '&fmt=json';
